...
function HomePage() {
  return <>
    <section className="relative min-h-[calc(100vh-5rem)] overflow-hidden bg-ink text-primary-foreground">
      ...
      <div className="relative border-t border-primary-foreground/15 bg-ink/70"><div className="page-shell grid md:grid-cols-3">{[[ShieldCheck,"Garantía y soporte local"],[BookOpenCheck,"Cumplimiento normativo"],[Globe2,"Cobertura nacional"]].map(([Icon,label],i)=><div key={label as string} className="flex items-center gap-3 border-primary-foreground/15 py-5 md:border-r md:px-7 first:md:pl-0 last:md:border-0"><Icon className="size-5 text-sky" /><span className="text-sm font-semibold">{label as string}</span></div>)}</div></div>
    </section>
    ...
    <section className="border-y border-border bg-panel py-24"><div className="page-shell grid gap-14 lg:grid-cols-[0.8fr_1.2fr]"><SectionHeading eyebrow="Ventaja Eduland" title="Decisiones técnicas con respaldo local" description="Acompañamos el proceso desde la definición del requerimiento hasta la continuidad operativa del laboratorio."/><div className="grid gap-8 sm:grid-cols-2">{[[Award,"Inversión eficiente","Alternativas competitivas evaluadas según desempeño y necesidad institucional."],[BookOpenCheck,"Enfoque curricular","Módulos configurables según competencias, planes de estudio y resultados de aprendizaje."],[Headphones,"Soporte especializado","Orientación local para instalación, operación, mantenimiento y continuidad de uso."],[CheckCircle2,"Atención integral","Un solo equipo para coordinar producto, logística, puesta en marcha y capacitación."]].map(([Icon,title,text])=><div key={title as string} className="border-l-2 border-sky pl-5"><Icon className="size-6 text-primary"/><h3 className="mt-4 font-bold text-ink">{title as string}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text as string}</p></div>)}</div></div></section>
    <QuoteBand />
  </>;
}
